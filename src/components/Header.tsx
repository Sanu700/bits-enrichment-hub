      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
        <div className="flex items-center justify-between h-18 md:h-20">
              ...
              <div key={item.path} className="relative">
                <button
                  ...
                  className={`relative px-3 py-2 rounded-xl text-[12px] font-body font-semibold transition-all duration-300 flex items-center gap-1 ${
                    isActive(item.path) ? "text-white bg-white/15 shadow-md border border-white/20" : "text-white/75 hover:text-white hover:bg-white/10"
                  }`}
                >
                  {isActive(item.path) && (
          ...
          <div className="hidden lg:flex items-center gap-2">
            <button className="p-2.5 rounded-xl text-white/75 hover:text-white hover:bg-white/10 transition-all duration-300">
                        ...
                        <div className="pl-9 pr-4 py-1 space-y-0.5">
                          ...
                          {item.children.map((child) => (
                            <Link
                              key={child.path}
                              to={child.path}
                              className="block py-2 px-3 rounded-lg text-[13px] font-body text-white/70 hover:text-white hover:bg-white/10 transition-colors"
                            >
                              {child.label}
                            </Link>
                          ))}
